...
const ALARM_PARAMETER_TYPES: { [key: string]: any } = {
  metrics: {
    label: 'Metrics',
    icon: <TrendingUpIcon />,
    parameters: {
...
const DashboardAlarmsTab: React.FC<DashboardAlarmsTabProps> = ({ device, metricsConfig = defaultMetricsConfig, onAlarmToggle }) => {
  ...
  const fetchAlarms = useCallback(async () => {
    ...
    try {
      ...
      const previousTriggeredIds = new Set(triggeredAlarms.map((a: any) => a.alarm_id));
      const now = Date.now();
  ...
  const handleAlarmTriggered = useCallback((alarm: any) => {
