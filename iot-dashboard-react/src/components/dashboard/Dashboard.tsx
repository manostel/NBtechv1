...
export default function Dashboard({ user, device, onLogout, onBack }: DashboardProps) {
  ...
  const [signalQualityData, setSignalQualityData] = useState<number[]>([]);
  const [lastSeen, setLastSeen] = useState<Date | null>(null);
  const [deviceStatus, setDeviceStatus] = useState<string | null>(null); // Initialize as null to avoid false status changes
  ...
  const [dataRangeWarning, setDataRangeWarning] = useState<string | null>(null);
  ...
  const [stateCheckTimeout, setStateCheckTimeout] = useState<NodeJS.Timeout | null>(null);
  const [pendingCommands] = useState<Map<string, any>>(new Map());
  const [activeCommands] = useState<Set<string>>(new Set());
  const [fetchTimeout, setFetchTimeout] = useState<NodeJS.Timeout | null>(null);
  const [pendingVerifications, setPendingVerifications] = useState<Map<string, any>>(new Map());
  ...
  // Shared state for charts and statistics tabs
  const [sharedChartsData, setSharedChartsData] = useState<any[]>([]);
  ...
  const isStatusStable = (currentStatus: string): boolean => {
  ...
  const handleVariableChange = (event: React.ChangeEvent<{ value: unknown }>, isOverview = false) => {
    const newVariables = event.target.value as string[];
    if (isOverview) {
  ...
  const fetchVariables = async () => {
    ...
    try {
      ...
      const newMetricsConfig: { [key: string]: any } = {};
      ...
      setError((error as Error).message || 'Failed to fetch available variables');
      setVariablesLoaded(false);
      return [];
    }
  };
    ...
    try {
      ...
      if (result.data && Array.isArray(result.data) && result.data.length > 0) {
        ...
        const processedData = result.data.map((item: any) => {
          ...
          Object.keys(processedItem).forEach((key: string) => {
  ...
  const handleExportData = () => {
    ...
    const timestamps = metricsData[Object.keys(metricsData)[0]]?.map((d: any) => d.timestamp) || [];
    
    timestamps.forEach((timestamp: any, index: number) => {
      const row = [
  ...
  const handleTimeRangeClick = (event: React.MouseEvent<HTMLElement>) => {
    setTimeRangeAnchor(event.currentTarget as HTMLElement);
  };
  ...
  const handleToggle1Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    setToggle1(event.target.checked);
    // Add command sending logic here
  };

  const handleToggle2Change = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSummaryTypeChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setSummaryType(event.target.value as string);
  };
  ...
  const handleTimeWindowChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setTimeWindow(event.target.value as string);
  };
  ...
  const handleAlarmUpdate = (updatedAlarms: any[]) => {
  ...
  const onCommandSend = async (command: string, params: any = {}) => {
    if (!device || !device.client_id) {
      console.error('No device or client_id available');
      return false;
    }
      ...
      case 0:
        return (
          <DashboardOverviewTab
            ...
            isLoading={isLoading}
            onVariableChange={(e: React.ChangeEvent<{ value: unknown }>) => handleVariableChange(e, true)}
      ...
      case 1:
        return (
          <DashboardChartsTab
            ...
            availableVariables={availableVariables}
            onVariableChange={(e: React.ChangeEvent<{ value: unknown }>) => handleVariableChange(e, false)}
  ...
  const handleDeviceStatusChange = useCallback((device: Device, oldStatus: string, newStatus: string) => {
    setDeviceStatus(newStatus);
  ...
  const handleBatteryLevel = useCallback((device: Device, batteryLevel: number) => {
    if (batteryLevel <= 20) {
      DeviceNotificationService.notifyBatteryLevel(device, batteryLevel);
    }
  }, []);

  const handleSignalStrength = useCallback((device: Device, signalStrength: number) => {
    if (signalStrength <= 30) {
      DeviceNotificationService.notifySignalStrength(device, signalStrength);
    }
  }, []);

  const handleThresholdExceeded = useCallback((device: Device, metric: string, value: number, threshold: number) => {
  ...
  return (
    ...
    >
      ...
      <SettingsDrawer 
        ...
        onShowClientIdChange={(e: React.ChangeEvent<HTMLInputElement>) => setShowClientId(e.target.checked)}
      />
