<<<<<<< HEAD:iot-dashboard/src/components/BluetoothControl.js
=======
...
// const STATUS_CHAR_UUID = "beb5483e-36e1-4688-b7f5-ea07361b26ab"; // Unused
...
interface BluetoothDevice extends BleDevice {
  name?: string;
}

const BluetoothControl: React.FC<BluetoothControlProps> = ({ onClose }) => {
  ...
  const attemptReconnect = async () => {
    ...
    if (storedDeviceId && storedDeviceName) {
      ...
      try {
        ...
        const device: BluetoothDevice = { deviceId: storedDeviceId, name: storedDeviceName };
  ...
  const initializeBluetooth = async () => {
    try {
      ...
      if (!Capacitor.isNativePlatform()) {
        // For development in browser, we can mock initialization but warn
        console.warn('Bluetooth is only available on native platforms (Android/iOS). Features will be limited in browser.');
  ...
  const startScan = async () => {
    ...
    if (!Capacitor.isNativePlatform()) {
      showNotification('Scanning only works on native devices', 'warning');
      return;
    }
  ...
  const readDeviceStates = async (deviceId: string) => {
    try {
  ...
  const connectToDevice = async (device: BluetoothDevice) => {
    if (!Capacitor.isNativePlatform()) {
      showNotification('Connection only works on native devices', 'warning');
      return;
    }
  ...
  const readDiagnosticData = async (deviceId: string) => {
    if (!deviceId) return;
    
    setIsLoadingDiagnostics(true);
    try {
      const diagnosticData = await BleClient.read(deviceId, SERVICE_UUID, MODEM_ISSUE_CHAR_UUID);
      const diagnosticText = new TextDecoder().decode(diagnosticData.value);
...
>>>>>>> dev-AWS-Connect-optimize:iot-dashboard/src/features/devices/components/BluetoothControl.tsx
