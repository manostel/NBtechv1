...
const StateCard: React.FC<StateCardProps> = ({ title, value, icon, color, unit, isBoolean }) => {
  const theme = useTheme();

  const getBooleanColor = (val: boolean) => {
    return val ? theme.palette.success.main : theme.palette.grey[400];
  };
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5, minHeight: 24 }}>
        {React.cloneElement(icon, {
          sx: {
            color: isBoolean ? getBooleanColor(value as boolean) : (color || theme.palette.primary.main),
            fontSize: 24,
            mr: 1
          }
        })}
...
const DeviceStateDisplay: React.FC<DeviceStateDisplayProps> = ({ deviceState, isLoading }) => {
  const theme = useTheme();
