...
const DashboardStatisticsTab: React.FC<DashboardStatisticsTabProps> = ({ 
  metricsData, 
  metricsConfig,
  timeRange,
  selectedVariables,
  ...
  const calculateStatistics = (data: any[], key: string) => {
  ...
  let dataPointsCount: number | null = null;
  ...
  return (
    <Box sx={{ py: 0, px: 0.5 }}>
      <SharedControls
        ...
        onApply={onApply}
        title={<span style={{ fontWeight: 400, fontSize: '1rem' }}>Statistics</span> as any}
      />
      <Grid container spacing={2}>
        {statsList}
      </Grid>
