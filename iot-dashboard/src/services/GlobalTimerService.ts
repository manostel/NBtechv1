<<<<<<< HEAD:iot-dashboard/src/services/GlobalTimerService.js
=======
...
class GlobalTimerService extends EventEmitter {
  ...
  updateDeviceStatus(deviceId: string, deviceData: DeviceData) {
    ...
    if (currentStatus !== newStatus) {
      ...
      if (shouldNotify) {
        console.log(`Device ${deviceId} status changed: ${currentStatus} → ${newStatus} (${timeDiff ? Math.round(timeDiff) : '?'}s ago)`);
  ...
  calculateDeviceStatus(deviceData: DeviceData | null) {
    try {
      ...
      if (isNaN(lastUpdateTime.getTime())) {
        ...
        const ddMMyyyyMatch = timestamp.match(/(\d{2})\/(\d{2})\/(\d{4}), (\d{1,2}):(\d{2}):(\d{2})/);
        if (ddMMyyyyMatch) {
        ...
        if (isNaN(lastUpdateTime.getTime())) {
          const mmDDyyyyMatch = timestamp.match(/(\d{2})\/(\d{2})\/(\d{4}), (\d{1,2}):(\d{2}):(\d{2})/);
          if (mmDDyyyyMatch) {
  ...
  getAllDeviceStatuses(): { [key: string]: string } {
...
>>>>>>> dev-AWS-Connect-optimize:iot-dashboard/src/services/GlobalTimerService.ts
