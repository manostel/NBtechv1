...
class GlobalTimerService extends EventEmitter {
  constructor() {
    ...
    // Constants
    this.STATUS_CHANGE_COOLDOWN = 5 * 60 * 1000; // 5 minutes
    this.NOTIFICATION_COOLDOWN = 10 * 60 * 1000; // 10 minutes
    this.STATUS_HISTORY_LIMIT = 10;
    this.INACTIVE_TIMEOUT_MINUTES = 7;
  }
  ...
  updateDeviceStatus(deviceId, deviceData) {
    ...
    if (currentStatus !== newStatus) {
      ...
      if (shouldNotify) {
        console.log(`Device ${deviceId} status changed: ${currentStatus} → ${newStatus} (${Math.round(timeDiff)}s ago)`);
  ...
  isStatusStable(deviceId, newStatus) {
  ...
  calculateDeviceStatus(deviceData) {
    try {
