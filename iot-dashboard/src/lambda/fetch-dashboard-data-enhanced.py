...
def get_last_timestamp(client_id, table_type='data'):
    """Get the most recent timestamp for a given client_id from specified table"""
    try:
        table = device_data_table if table_type == 'data' else device_status_table
        logger.info(f"Querying for last timestamp of client_id: {client_id} from {table_type} table")
        ...
        if response.get('Items') and len(response['Items']) > 0:
            ...
            logger.warning(f"No data found for client_id {client_id} in {table_type} table")
            return None
    except Exception as e:
        logger.error(f"Error getting last timestamp from {table_type} table: {str(e)}")
        return None

def fetch_chunk(client_id, start_time, end_time, selected_variables, include_state, table_type='data'):
    ...
    try:
        ...
        logger.error(f"Error fetching chunk from {table_type} table: {str(e)}")
        raise
...
def fetch_data_in_chunks(client_id, start_time, end_time=None, selected_variables=None, table_type='data'):
    ...
    try:
        ...
        logger.info(f"Starting data fetch for client_id: {client_id} from {table_type} table")
        ...
        if not all_items:
            logger.warning(f"No data found for client_id: {client_id} in the specified time range from {table_type} table")
            ...
            # Try a wider time range as fallback
            logger.info(f"Trying wider time range as fallback...")
            wider_start = start_time - timedelta(hours=24)  # Go back 24 hours
            wider_end = end_time + timedelta(hours=24)     # Go forward 24 hours
            
            logger.info(f"Fallback query: {wider_start.isoformat()} to {wider_end.isoformat()}")
            ...
            try:
                fallback_response = table.query(
                    ...
                    ExpressionAttributeValues={
                        ':client_id': client_id,
                        ':start_time': wider_start.isoformat(),
                        ':end_time': wider_end.isoformat()
                    },
                    ExpressionAttributeNames={
                        '#ts': 'timestamp'
                    },
                    ScanIndexForward=True,
                    Limit=100  # Limit fallback query
                )
...
def calculate_summary_statistics(data, metrics, table_type='data'):
...
def lambda_handler(event, context):
    ...
    try:
        ...
        table_type = body.get('table_type', 'data')  # 'data' or 'status'
        ...
        # Validate table_type
        if table_type not in ['data', 'status']:
            return {
                'statusCode': 400,
                'headers': {
                    'Access-Control-Allow-Origin': '*',
                ...
                'body': json.dumps({
                    'error': 'Invalid table_type',
                    'message': 'table_type must be either "data" or "status"'
                })
            }
        ...
        if not items:
            return {
                ...
                'body': json.dumps({
                    'data': [],
                    'summary': {
                        'message': f'No data available for the selected time range from {table_type} table',
