<<<<<<< HEAD:iot-dashboard/src/utils/RealTimeSubscriptionDetector.js
=======
...
class RealTimeSubscriptionDetector extends EventEmitter {
  ...
  // @ts-ignore - unused but kept for structure
  private subscriptions = new Map();
  private lastValues = new Map(); // Track last known values
  // @ts-ignore - unused but kept for structure
  private subscriptionChecks = new Map(); // Cache subscription data
  ...
  async getActiveSubscriptions(userEmail: string): Promise<Subscription[]> {
    try {
      ...
      return result.subscriptions?.filter((sub: Subscription) => sub.enabled) || [];
    } catch (error) {
  ...
  groupSubscriptionsByDevice(subscriptions: Subscription[]): Map<string, Subscription[]> {
  ...
  async checkDeviceChanges(deviceId: string, subscriptions: Subscription[]) {
    try {
  ...
  evaluateSubscriptionCondition(subscription: Subscription, currentValue: any) {
    const { condition_type, threshold_value } = subscription;
    
    // Handle null/undefined threshold for certain conditions
    const threshold = threshold_value ?? 0;
  ...
  formatChangeMessage(subscription: Subscription, currentValue: any, previousValue: any) {
  ...
  getChangeType(currentValue: any, previousValue: any) {
  ...
  getNotificationSeverity(subscription: Subscription): 'success' | 'error' | 'info' | 'warning' {
  ...
  storeNotification(notification: RealTimeNotification) {
    try {
  ...
  handleRealtimeData(data: any) {
    // Handle real-time data from MQTT bridge
    const { device_id, parameter_name, value } = data;
  ...
  getSubscriptionsForDeviceParameter(deviceId: string, parameterName: string): Subscription[] {
...
>>>>>>> dev-AWS-Connect-optimize:iot-dashboard/src/utils/RealTimeSubscriptionDetector.ts
